...
with tab1:
    ...
    with st.form("manual_input_form"):
        ...
        with c1:
            state = st.selectbox("State", ["Assam", "Manipur", "Meghalaya", "Mizoram",
        ...
        with c1:
            water_source = st.selectbox("Water Source", ["River", "Pond", "Open Well", "Rainwater", "Tanker", "Borewell", "Piped"])
        with c2:
            water_treatment = st.selectbox("Water Treatment", ["Untreated", "Boiled", "Filtered", "Chlorinated"])
        with c3:
    ...
    if submitted:
        ...
        with st.spinner("Predicting disease..."):
            try:
                ...
                result   = pipeline.predict(pd.DataFrame([patient_dict]))
...
with tab3:
    if st.session_state.prediction_result is None:
        ...
        st.markdown(
            f'<div class="{css_class}">🦠 {display_disease}<br>'
            f'<small style="font-size:1rem">Confidence: {prob:.1f}%</small></div>',
            unsafe_allow_html=True
        )
        st.markdown("")
...
<b style="color:#856404;">⚠️ High Confidence Prediction — Medical Consultation Recommended</b><br><br>
<span style="color:#333;"><b>Recommended Specialist:</b> {doc['specialist']}</span><br>
<span style="color:#333;"><b>Urgency:</b> {doc['urgency']}</span><br><br>
<span style="color:#333;"><b>Emergency Numbers:</b> 📞 <b>108</b> (Ambulance) &nbsp;|&nbsp; 📞 <b>104</b> (Health Helpline)</span>
</div>
""", unsafe_allow_html=True)
    ...
    if st.session_state.prediction_result is None:
        ...
        if st.session_state.advisory is None:
            if st.button("🤖 Generate WHO Health Advisory", use_container_width=True, type="primary"):
                with st.spinner("Generating advisory using Gemini AI + WHO guidelines... (15-20 seconds)"):
                    try:
            ...
            # Display advisory using st.markdown — auto adapts to dark/light theme
